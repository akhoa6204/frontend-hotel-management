...
const RoomDescription = ({ name, description, capacity, basePrice, discount, amenities = [], rating, totalReviews, handleBookingRoom }: Props) => {
  ...
  return <Box component="section" sx={{ pt: { xs: 4.5, md: 6 }, pb: { xs: 5, md: 7 } }}>
    <Grid container spacing={{ xs: 3, md: 7 }} alignItems="flex-end" sx={{ pb: { xs: 4, md: 5 }, borderBottom: "1px solid #dcd9d1" }}>
      <Grid size={{ xs: 12, md: 8 }}><Typography sx={{ color: "primary.main", letterSpacing: 2, fontSize: 12, fontWeight: 700 }}>PHÒNG NGHỈ DIAMOND SEA</Typography><Typography component="h1" sx={{ mt: 1, fontFamily: "Georgia, serif", fontSize: { xs: 38, md: 50 }, fontWeight: 400, lineHeight: 1.12 }}>{name}</Typography>{description && <Typography color="text.secondary" sx={{ mt: 1.75, maxWidth: 720, fontSize: 16, lineHeight: 1.7 }}>{description}</Typography>}<Stack direction="row" flexWrap="wrap" gap={2.5} sx={{ mt: 2.25, color: "text.secondary" }}><Stack direction="row" spacing={.75} alignItems="center"><PeopleAltOutlinedIcon fontSize="small" /><Typography variant="body2">Tối đa {capacity} khách</Typography></Stack>{rating > 0 && <Stack direction="row" spacing={.5} alignItems="center"><StarRoundedIcon sx={{ color: "rating.main", fontSize: 20 }} /><Typography variant="body2">{rating.toFixed(1)} · {totalReviews} đánh giá</Typography></Stack>}</Stack></Grid>
    ...
    <Grid container spacing={{ xs: 5, md: 7 }} sx={{ pt: { xs: 5, md: 7 } }}>
      ...
      <Grid size={{ xs: 12, md: 4 }}>
        <Paper elevation={0} sx={{ position: { md: "sticky" }, top: { md: 104 }, p: { xs: 3, md: 3.5 }, borderRadius: 1, bgcolor: "background.paper", border: "1px solid", borderColor: "divider", boxShadow: "0 14px 36px rgba(16,43,56,.09)" }}>
          <Typography variant="body2" color="text.secondary">Giá mỗi đêm từ</Typography><Typography sx={{ mt: .25, fontSize: 30, fontWeight: 600, color: "text.primary" }}>{fmtVND(nightlyPrice)} ₫</Typography><Divider sx={{ my: 2.5 }} />
