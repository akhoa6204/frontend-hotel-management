    ...
    >
      <Box sx={{ position: "relative", width: 1, aspectRatio: { xs: "16 / 7", sm: "16 / 6", md: "16 / 9" }, overflow: "hidden", bgcolor: "#e3e0d9" }}>
        <Box
          component="img"
          src={image}
          alt={suitableImage?.alt || `Phòng ${booking.room.roomType.name}`}
      ...
      <Box sx={{ p: { xs: 2.5, sm: 3, md: 3.25 } }}>
        ...
        <Typography component="h2" sx={{ mt: 1, fontFamily: "Georgia, serif", fontSize: { xs: 25, md: 28 }, lineHeight: 1.2, color: "#183746" }}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", columnGap: 1.5 }}>
          <Box>
            ...
            <Typography fontWeight={700} color="#183746" sx={{ mt: 0.5, whiteSpace: "nowrap" }}>{formatDate(booking.checkInDate)}</Typography>
          </Box>
          <Stack alignItems="center" spacing={0.5} sx={{ minWidth: 64, color: "text.secondary" }}>
            ...
            <Typography variant="caption" sx={{ whiteSpace: "nowrap" }}>{nights} đêm</Typography>
          </Stack>
          <Box sx={{ textAlign: "right" }}>
            <Typography variant="caption" color="text.secondary">Trả phòng</Typography>
            <Typography fontWeight={700} color="#183746" sx={{ mt: 0.5, whiteSpace: "nowrap" }}>{formatDate(booking.checkOutDate)}</Typography>
          </Box>
        </Box>

        <Divider sx={{ my: 3, borderColor: "#dedbd4" }} />

        <Typography component="h3" sx={{ fontSize: 17, fontWeight: 700, color: "#183746" }}>Thông tin khách</Typography>
        <Stack spacing={1.1} sx={{ mt: 1.75, color: "#425866" }}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="baseline" spacing={2}>
          <Typography color="text.secondary">Tổng kỳ nghỉ</Typography>
          <Typography sx={{ fontSize: 21, fontWeight: 750, color: "#183746", whiteSpace: "nowrap" }}>{fmtVND(finalTotal)} VND</Typography>
        </Stack>
      </Box>
    </Box>
  );
